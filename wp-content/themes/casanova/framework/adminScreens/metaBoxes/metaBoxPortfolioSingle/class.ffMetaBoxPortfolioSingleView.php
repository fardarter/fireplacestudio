...
protected function _requireAssets() {
	...
	protected function _render( $post ) {
		...
		$s = $fwc->getOptionsFactory()->createOptionsHolder('ffComponent_Theme_MetaboxPortfolio_SingleView')->getOptions();//createStructure('portfolio');




		$value = $fwc->getDataStorageFactory()->createDataStorageWPPostMetas_NamespaceFacade(  $post->ID )->getOption('portfolio_single_options');

		$printer = $fwc->getOptionsFactory()->createOptionsPrinterBoxed( $value, $s );
		$printer->setNameprefix('portfolio_single_options');
		$printer->walk();
	...
	protected function _save( $postId ) {
		...
		$value = $fwc->getOptionsFactory()->createOptionsPostReader()->getData( 'portfolio_single_options');

		$saver->setOption( 'portfolio_single_options' , $value );
